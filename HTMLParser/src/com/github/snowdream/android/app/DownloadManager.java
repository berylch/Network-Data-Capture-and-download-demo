...
public class DownloadManager {
	...
	static public DownloadManager gDownloadManager = null;
	...
	public DownloadManager(Context context) {
		...
		try {
			mItems = iSql.queryAllDownloadTask();
			android.util.Log.i("lxmlxmlxm", "" + mItems.size());
	...
	private void pushItem(DownloadTask task) {
		if (task == null) {
			return;
		}
		if (mItems == null) {
			mItems = new ArrayList<DownloadTask>();
		}
		android.util.Log.i("pushItem", "" + mItems.size());
		for (DownloadTask item : mItems) {
			if (item.getUrl().matches(task.getUrl())) {
				return;// 存在时，直接返回
			}
		}
		mItems.add(task);
	...
	private void popItem(DownloadTask task) {
		if (task == null) {
			return;
		}
		if (mItems == null) {
		...
		android.util.Log.i("popItem", "" + mItems.size());
		for (DownloadTask item : mItems) {
			if (item.getUrl().matches(task.getUrl())) {
				mItems.remove(item);
				android.util.Log.i("popItem", "" + mItems.size());
	...
	private static Executor exec = new ThreadPoolExecutor(2, 5, 10,
	...
	public static DownloadManager getInstance(Context context) {
		if (gDownloadManager == null) {
			...
			gDownloadManager = new DownloadManager(context);
			android.util.Log.i("lxmlxmlxm", "55555");
		}
		gDownloadManager.context = context; // 修整context
	...
	private void OnResult(final POST_MESSAGE message, final DownloadTask task,
			final DownloadListener listener, final Integer code) {
	...
	public void add(DownloadTask task, DownloadListener listener) {
		...
		if (task == null || !task.isValid()) {
			OnResult(POST_MESSAGE.ERROR, task, listener,
					DownloadException.DOWNLOAD_TASK_NOT_VALID);
			return;
		}
		...
		try {
			...
			if (temptask == null || !temptask.isValid()
					|| !temptask.isComplete()) {
				if (task.isComplete()) {
				...
				OnResult(POST_MESSAGE.FINISH_ONLY, task, listener,
						DownloadException.DOWNLOAD_TASK_NOT_VALID);
			}
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public boolean start(DownloadTask task, DownloadListener listener) {
		...
		if (task == null) {
			OnResult(POST_MESSAGE.ERROR, task, listener,
					DownloadException.DOWNLOAD_TASK_NOT_VALID);
			android.util.Log.i("lxmlxm", "error");
			return ret;
		}
	...
	private enum POST_MESSAGE {
		ADD, DELETE, ERROR, START, STOP, FINISH, FINISH_ONLY
	}
}
