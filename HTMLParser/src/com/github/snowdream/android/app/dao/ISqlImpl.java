...
public class ISqlImpl implements ISql {
	...
	public List<DownloadTask> queryDownloadTask(HashMap<String, Object> Map)
			throws SQLException {
		if (Map.isEmpty()) {
			return null;
		}
		Dao<DownloadTask, Integer> taskDao = getHelper().getTaskDao();
		Log.i("" + Integer.valueOf(String.valueOf(Map.get("status"))));
		if (Integer.valueOf(String.valueOf(Map.get("status"))) == DownloadStatus.STATUS_NONE) {
			return taskDao.queryForAll();
		} else {
			return taskDao.queryForFieldValuesArgs(Map);
		}
	}

	public List<DownloadTask> queryAllDownloadTask() throws SQLException {
		Dao<DownloadTask, Integer> taskDao = getHelper().getTaskDao();
		return taskDao.queryForAll();
	...
	/*
	 * @Override public List<DownloadTask> queryDownloadTasksFromAlbum(Album
	 * album) throws SQLException { List<DownloadTask> list = null; Album talbum
	 * = null;
	 * 
	 ...
	 * 
	 * if (talbum == null) { return list; }
	 * 
	 * list = new ArrayList<DownloadTask>();
	 * 
	 * for (DownloadTask task : talbum.getDownloadTasks()) { list.add(task); }
	 * 
	 * return list; }
	 * 
	 * @Override public List<DownloadTask> queryDownloadTasksFromAlbums(Albums
	 * albums) throws SQLException { List<DownloadTask> list = null; Albums
	 * talbums = null;
	 * 
	 * if (albums == null) { return list; }
	 * 
	 * Dao<Albums, String> albumsDao = getHelper().getAlbumsDao();
	 * 
	 * talbums = albumsDao.queryForSameId(albums);
	 * 
	 * if (talbums == null) { return list; }
	 * 
	 * list = new ArrayList<DownloadTask>();
	 * 
	 * for (Album album : talbums.getAlbums()) { List<DownloadTask> alist =
	 * queryDownloadTasksFromAlbum(album);
	 * 
	 * if (alist != null) { list.addAll(alist); } }
	 * 
	 * return list; }
	 */
