...
public class AsycDownloadTask extends
		AsyncTask<DownloadTask, Integer, DownloadTask> {

	private static final int MESSAGE_POST_ERROR = 0x1;

	private static final int MESSAGE_POST_ADD = 0x2;

	private static final int MESSAGE_POST_FINISH = 0x3;
	/**
	 * http default
	 */
	...
	private static final String STORE_PATH = "/mnt/sdcard/downloader/";
	...
	private void OnFinish(DownloadTask task) {
		if (listener != null && listener instanceof DownloadListener) {
			((DownloadListener) listener).onFinish(task);
		}
	}
	...
	private void SendError(DownloadTask task, Integer code) {
		...
		sHandler.obtainMessage(MESSAGE_POST_ERROR,
				new AsyncTaskResult(this, task, code)).sendToTarget();
	}

	private void SendAdd(DownloadTask task) {
		sHandler.obtainMessage(MESSAGE_POST_ADD,
				new AsyncTaskResult(this, task, -1)).sendToTarget();
	}

	private void SendFinish(DownloadTask task) {
		sHandler.obtainMessage(MESSAGE_POST_FINISH,
				new AsyncTaskResult(this, task, -1)).sendToTarget();
	}
	...
	protected DownloadTask doInBackground(DownloadTask... tasks) {
		...
		android.util.Log.i("DownloadTask", task.getName());
		...
		try {
			...
			while (true) {
				...
				case HttpURLConnection.HTTP_PARTIAL:
					...
					String transfer_encoding = connection
				...
				if (TextUtils.isEmpty(filename)) {
					...
					if (disposition != null) {
						...
						filename = disposition.substring(
								...
								endIdx > 0 ? endIdx : disposition.length());
					} else {
			...
			while ((nRead = in.read(buffer, 0, 1024)) > 0) {
				...
				if (size != 0) {
					...
					long elapsedtime = currentTime - time_now;

					if (elapsedtime > 500) {
						time_now = currentTime;
//						android.util.Log.i("3333333", ""
//								+ ((curSize - size_cur) / elapsedtime) + "-"
//								+ "cur size:" + (curSize) + "    size_cur:"
//								+ (size_cur) + "    elapsedtime"
//								+ (elapsedtime));

						spead = (int) ((curSize - size_cur) / elapsedtime);
						size_cur = curSize;
						task.setSpead(spead);// 添加下载速率
	...
	private static class AsyncTaskResult {
		...
		AsyncTaskResult(@SuppressWarnings("rawtypes") AsyncTask task,
				DownloadTask downloadtask, Integer data) {
			mTask = task;
			mData = data;
	...
	private static class InternalHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			if (result == null || result.mTask == null
					|| result.mTask.isCancelled()) {
			...
			case MESSAGE_POST_ERROR:
				((AsycDownloadTask) result.mTask).OnError(result.mDownloadTask,
						result.mData);
				break;
			...
			case MESSAGE_POST_FINISH:
				((AsycDownloadTask) result.mTask)
						.OnFinish(result.mDownloadTask);
				break;
			default:
				break;
			}
